...
server <- function(input, output, session) {
  ...
  counters$grade <- 0
  counters$student <- sprintf("S%03d", floor(runif(1,min=1,max=999)))
  counters$date <- NULL
  counters$startTime <- NULL
  counters$submitTime <- NULL
  counters$detailUpdated <- 0
  ...
  observeEvent(input$submitButton,{
    ...
    counters$detailUpdated <- counters$detailUpdated + 1
  ...
  output$testDetail <- renderTable({
      
      counters$detailUpdated  # for update the table.
